...
class TabVehicleInfo(VehicleSpecial):
    ...
    def populate_vehicle(self):#Moves to create vehicle by adding Stock# to textbox then clicking the populate button
        ...
        self.driver.find_element(By.ID, "acf-field_56917bb83947f-stock").click()# Click Offer Applies To - Stock(one unit)

        element = self.driver.find_element(By.ID, "acf-field_56549cefdeb1a")
        actions = ActionChains(self.driver)
        actions.move_to_element(element).perform()
        Today.time_taken(JsTextBox.fix_text_box, self.driver, self.vehicle.StockNumber, self.driver.find_element(By.ID, "acf-field_56549cefdeb1a"))# Send Stock Number to Vehicle Stock Text Box
        self.driver.find_element(By.ID, "populate_vehicle").click()# Click Populate Stock Number - Image will auto load

    @staticmethod
    def populate_title_boxes(self):#Edits the title boxes for both the offer and backend
        while True:
            try: # Check for if vehicle exists
                element = self.driver.find_element(By.NAME, "post_title")# add vehicle title to Title Text Box
                actions = ActionChains(self.driver)
                actions.move_to_element(element).perform()
                ...
                self.driver.find_element(By.ID, "acf-field_5575ca339b200").clear()# clear vehicle title 
                element = self.driver.find_element(By.ID, "acf-field_5575ca339b200")# replace vehicle title with fixed one
                Today.time_taken(JsTextBox.fix_text_box, self.driver, f'New {self.vehicle.Year} {self.vehicle.MakeName} {self.vehicle.ModelName} {self.vehicle.Trim}', element)

                self.driver.find_element(By.CSS_SELECTOR, "li:nth-child(2) b").click()# Click Offer Applies To - Inventory
                Today.time_taken(JsTextBox.fix_text_box, self.driver, self.vehicle.StockNumber, self.driver.find_element(By.ID, "acf-diso_vehicle_stock"))# Send Keys to Secondary Stock Number Box 
                break 
